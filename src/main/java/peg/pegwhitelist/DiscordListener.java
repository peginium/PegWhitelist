...
public class DiscordListener extends ListenerAdapter implements EventListener, Listener {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (args[0].equalsIgnoreCase("p!whitelist")) {
            if (args.length < 2) {
                ...
                info.setTitle(savedArgs + " " + main.titleDc);
        ...
        if (args[0].equalsIgnoreCase("p!blacklist")) {
            if (args.length < 2) {
                event.getChannel().sendMessage(main.errorDc).queue();
            } else {
                if (main.blPerm) {
                ...
                savedArgs = args[1];
                EmbedBuilder info = new EmbedBuilder();
                info.setTitle(savedArgs + " " + main.remtitleDc);
                ...
                System.out.println(savedArgs + " removed from whitelist");
